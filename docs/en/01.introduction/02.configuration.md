...
Below is the full configuration available with defaults values:
    ...
    "example" => [
        ...
        "config" => [
            ...
            "button_type"   => "info",
            "mode"          => "dropdown",
            "handler"       => "Anomaly\IconFieldType\IconFieldTypeOptions@handle"
        ]
    ]
...
Icon options can be organized into `groups` by nesting the options into keyed groups. The `key` is translated as the group's label.
...
Option handlers can also a handler with a closure:
    ...
    "example" => [
        "config" => [
            "handler" => function (IconFieldType $fieldType) {
...
The handler string is called via Laravel's service container. The `IconFieldType $fieldType` is passed as an argument.
